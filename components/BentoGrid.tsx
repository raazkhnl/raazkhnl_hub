    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="md:col-span-2 obsidian-card p-10 rounded-[2.5rem] flex flex-col justify-center relative overflow-hidden group border-white/5">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-[11px] font-black mono text-slate-200 tracking-[0.4em] uppercase">Mission_Directive</h3>
        </div>
      ...
      <div className="md:col-span-1 obsidian-card p-8 rounded-[2rem] flex flex-col items-center justify-center text-center group transition-all border-white/5">
        ...
        <p className="text-[10px] mono text-cyan-400 font-black uppercase tracking-widest mb-1">IOE Pulchowk</p>
        <p className="text-[9px] text-slate-500 font-black italic tracking-tighter uppercase">'24 BE COMPUTER</p>
      </div>
      ...
      <div className="md:col-span-3 obsidian-card p-10 rounded-[3rem] border-white/5 relative group overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-10">
          <div className="w-1.5 h-1.5 rounded-full bg-white opacity-40"></div>
          <h3 className="text-[11px] font-black mono text-slate-200 tracking-[0.4em] uppercase">Stack_Matrix.db</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-12">
          {SKILLS.map((cat, i) => (
            <div key={i} className="relative">
              <p className="text-[10px] mono text-slate-600 font-black uppercase tracking-[0.3em] mb-6 flex items-center gap-4">
                {cat.category}
                <span className="h-px flex-1 bg-white/5"></span>
              </p>
              <div className="flex flex-wrap gap-2.5">
                {cat.skills.map((s, si) => (
                  <span key={si} className="px-4 py-2 bg-black/40 border border-white/5 rounded-xl text-[11px] mono text-slate-400 hover:text-white hover:border-white/20 transition-all cursor-default font-black uppercase tracking-tight">
                    {s}
                  </span>
                ))}
              </div>
